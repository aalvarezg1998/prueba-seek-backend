...
class TaskControllerTest {
    ...
    void setUp() {
        ...
        jwtToken = jwtService.generateToken(userId);
    ...
    void getAllTasks_shouldReturnTasks() throws Exception {
        when(taskService.getAllTasks(userId)).thenReturn(List.of(taskResponse));
    ...
    void getTaskById_shouldReturnTask() throws Exception {
        when(taskService.getTaskById(taskId, userId)).thenReturn(taskResponse);
    ...
    void getTaskById_whenNotFound_shouldReturn404() throws Exception {
        when(taskService.getTaskById(taskId, userId)).thenThrow(new TaskNotFoundException(taskId));
    ...
    void createTask_shouldReturnCreatedTask() throws Exception {
        ...
        when(taskService.createTask(any(CreateTaskRequest.class), eq(userId))).thenReturn(taskResponse);
    ...
    void createTask_withInvalidData_shouldReturn422() throws Exception {
        CreateTaskRequest request = new CreateTaskRequest("", "Description", "INVALID");
    ...
    void updateTask_shouldReturnUpdatedTask() throws Exception {
        UpdateTaskRequest request = new UpdateTaskRequest("Updated", null, "IN_PROGRESS");
        TaskResponse updated = new TaskResponse(
                taskId, "Updated", "Test Description", "IN_PROGRESS",
                LocalDateTime.now(), LocalDateTime.now()
        );
        when(taskService.updateTask(eq(taskId), any(UpdateTaskRequest.class), eq(userId))).thenReturn(updated);
